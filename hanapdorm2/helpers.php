...
function send_reset_email(string $email, string $token): bool
{
    ...
    // Get the base directory by stripping out the auth segment from the script path.
    // This guarantees we never duplicate '/auth/' in the final URL.
    $rootDir = preg_replace('#/auth/.*$#', '', $_SERVER['SCRIPT_NAME'] ?? '');
    $resetLink = $scheme . '://' . $host . $rootDir . '/auth/reset-password.php?token=' . urlencode($token);
